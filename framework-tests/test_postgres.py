...
PARAMS: Dict[str, Union[str, int]] = {
    "host": os.environ.get("VASTBASE_HOST", "localhost"),
    "user": os.environ.get("VASTBASE_USER", "vexdb"),
...
# Vastbase availability check via pyvastbase
vastbase_not_available = False
try:
    import pyvastbase  # noqa: F401
except ImportError:
...
@pytest.fixture()
def pg(db: None) -> Any:
    pg = VastbaseVectorStore.from_params(
        **PARAMS,  # type: ignore
        database=TEST_DB,
...
@pytest.fixture()
def pg_hnsw_multiple(db_hnsw: None) -> Generator[List[VastbaseVectorStore], None, None]:
    """
    This creates multiple instances of VastbaseVectorStore.
    """
    pgs = []
    for _ in range(2):
        pg = VastbaseVectorStore.from_params(
...
@pytest.mark.skipif(vastbase_not_available, reason="postgres db is not available")
@pytest.mark.asyncio
async def test_instance_creation(db: None) -> None:
    ...
    assert isinstance(pg, VastbaseVectorStore)
...
def second_table(db):
    ...
    engine = create_engine(
        f"postgresql+pyvastbase://{PARAMS['user']}:{PARAMS['password']}@{PARAMS['host']}:{PARAMS['port']}/{TEST_DB}",
        echo=False,
    )
...
async def test_add_to_db_and_query(
    pg_fixture: VastbaseVectorStore, node_embeddings: List[TextNode], use_async: bool
) -> None:
    if use_async:
        await pg_fixture.async_add(node_embeddings)
    else:
        pg_fixture.add(node_embeddings)
    assert isinstance(pg_fixture, VastbaseVectorStore)
...
async def test_query_hnsw(
    pg_hnsw: VastbaseVectorStore, node_embeddings: List[TextNode], use_async: bool
):
    if use_async:
    ...
    assert isinstance(pg_hnsw, VastbaseVectorStore)
...
async def test_sparse_query(
    ...
    assert isinstance(pg_hybrid, VastbaseVectorStore)
...
@pytest.mark.skipif(vastbase_not_available, reason="postgres db is not available")
@pytest.mark.asyncio
@pytest.mark.skip(reason="pgvector tsquery internals — not applicable to pyvastbase")
...
@pytest.mark.parametrize("use_async", [True, False])
async def test_hybrid_query(
    pg_hnsw_hybrid: VastbaseVectorStore,
    hybrid_node_embeddings: List[TextNode],
    use_async: bool,
) -> None:
    if use_async:
    ...
    assert isinstance(pg_hnsw_hybrid, VastbaseVectorStore)
...
def test_hybrid_query_fails_if_no_query_str_provided(
    pg_hybrid: VastbaseVectorStore, hybrid_node_embeddings: List[TextNode]
) -> None:
    q = VectorStoreQuery(
...
async def test_add_to_db_and_query_index_nodes(
    pg_fixture: VastbaseVectorStore, index_node_embeddings: List[BaseNode], use_async: bool
) -> None:
    if use_async:
...
async def test_delete_nodes(
    pg_fixture: VastbaseVectorStore, node_embeddings: List[BaseNode], use_async: bool
) -> None:
    if use_async:
        await pg_fixture.async_add(node_embeddings)
    else:
...
async def test_hnsw_index_creation(
    pg_hnsw_multiple: List[VastbaseVectorStore],
    node_embeddings: List[TextNode],
    use_async: bool,
) -> None:
    """
    This test will make sure that creating multiple VastbaseVectorStores handles db initialization properly.
    """
    ...
    # these are the actual table and index names that VastbaseVectorStore automatically created
    ...
    with pyvastbase.connect(**PARAMS, database=TEST_DB_HNSW) as hnsw_conn:
...
async def test_custom_engines(db: None, node_embeddings: List[TextNode]) -> None:
    """Test that VastbaseVectorStore works correctly with custom engines."""
    ...
    # Create VastbaseVectorStore with custom engines
    pg = VastbaseVectorStore(
        embed_dim=TEST_EMBED_DIM,
        engine=engine,
...
@pytest.mark.skipif(vastbase_not_available, reason="postgres db is not available")
@pytest.mark.skip(reason="tests SQLAlchemy engine-only ValueError — VastbaseVectorStore accepts engine but ignores it")
def test_custom_sync_engine_only(db: None, node_embeddings: List[TextNode]) -> None:
    """Test that VastbaseVectorStore works correctly with only a custom sync engine."""
    ...
    # Create VastbaseVectorStore with custom sync engine only
    with pytest.raises(ValueError):
        _ = VastbaseVectorStore(
            embed_dim=TEST_EMBED_DIM,
            engine=engine,
        )
...
@pytest.mark.skipif(vastbase_not_available, reason="postgres db is not available")
@pytest.mark.asyncio
@pytest.mark.skip(reason="tests SQLAlchemy async_engine-only ValueError — VastbaseVectorStore accepts async_engine but ignores it")
async def test_custom_async_engine_only(
    db: None, node_embeddings: List[TextNode]
) -> None:
    """Test that VastbaseVectorStore works correctly with only a custom async engine."""
    from sqlalchemy.ext.asyncio import create_async_engine
    ...
    # Create VastbaseVectorStore with custom async engine only
    with pytest.raises(ValueError):
        _ = VastbaseVectorStore(
            embed_dim=TEST_EMBED_DIM,
            async_engine=async_engine,
...
async def test_indexed_metadata(
    pg_indexed_metadata: VastbaseVectorStore,
    hybrid_node_embeddings: List[TextNode],
) -> None:
...
async def test_custom_query(
    use_async: bool, pg_custom_query: VastbaseVectorStore, node_embeddings: List[TextNode]
) -> None:
...
@pytest.mark.skipif(vastbase_not_available, reason="postgres db is not available")
@pytest.mark.asyncio
@pytest.mark.skip(reason="queries PostgreSQL system catalog (pg_class/pg_am/pg_indexes) — not applicable")
async def test_gin_index_creation_in_database(
    pg_gin_array_indexed: VastbaseVectorStore,
...
async def test_custom_sparse_query(
    use_async: bool,
    pg_custom_query: VastbaseVectorStore,
    hybrid_node_embeddings: List[TextNode],
) -> None:
...
def _create_mock_pg_vector_store() -> MagicMock:
    """Create a mock VastbaseVectorStore with minimal setup for testing _mmr_query."""
    mock_store = MagicMock(spec=VastbaseVectorStore)
    mock_store.hnsw_kwargs = None

    mock_store._db_rows_to_query_result = (
        VastbaseVectorStore._db_rows_to_query_result.__get__(mock_store)
    )
    mock_store._prepare_mmr_query = VastbaseVectorStore._prepare_mmr_query.__get__(mock_store)
    mock_store._mmr_rerank_results = VastbaseVectorStore._mmr_rerank_results.__get__(
        mock_store
    )
    return mock_store
...
async def _call_mmr(mock_store, query, use_async, **kwargs):
    if use_async:
        return await VastbaseVectorStore._async_mmr_query(mock_store, query, **kwargs)
    else:
        return VastbaseVectorStore._mmr_query(mock_store, query, **kwargs)
...
def test_get_query_session_settings_empty_when_no_kwargs():
    ...
    settings = VastbaseVectorStore._get_query_session_settings(mock_store)
    assert settings == []
...
def test_get_query_session_settings_ivfflat_probes():
    ...
    settings = VastbaseVectorStore._get_query_session_settings(mock_store, ivfflat_probes=10)
...
def test_get_query_session_settings_hnsw_override_from_kwargs():
    ...
    settings = VastbaseVectorStore._get_query_session_settings(mock_store, hnsw_ef_search=200)
...
def test_get_query_session_settings_both_ivfflat_and_hnsw():
    ...
    settings = VastbaseVectorStore._get_query_session_settings(mock_store, ivfflat_probes=7)
...
def test_get_query_session_settings_casts_to_int():
    ...
    settings = VastbaseVectorStore._get_query_session_settings(
...
async def test_query_routes_to_mmr(use_async):
    ...
    if use_async:
        result = await VastbaseVectorStore.aquery(mock_store, q)
        mock_store._async_mmr_query.assert_called_once_with(q)
    else:
        result = VastbaseVectorStore.query(mock_store, q)
...
def test_prepare_mmr_query_query_threshold_takes_precedence():
    ...
    _, threshold = VastbaseVectorStore._prepare_mmr_query(
...
def test_prepare_mmr_query_threshold_none_when_both_absent():
    ...
    _, threshold = VastbaseVectorStore._prepare_mmr_query(mock_store, query)
